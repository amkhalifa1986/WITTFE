...
export const LostFound = () => {
  ...
  const handleCreatePost = async (e) => {
    ...
    try {
      ...
      toast(t('Report submitted successfully!'), 'success');
      fetchPosts();
    } catch (err) {
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      ...
      setPostComments(res.data.comments || []);
      setNewComment('');
      toast(t('Comment added successfully!'), 'success');
    } catch (err) {
  ...
  const handleMarkResolved = async (postId) => {
    const isConfirmed = await confirm('Mark this report as resolved? It will close comments.');
    if (!isConfirmed) return;
    try {
